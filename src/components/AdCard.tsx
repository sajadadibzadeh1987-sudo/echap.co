...
// نرمال‌سازی آدرس تصویر برای همه حالت‌ها
function resolveImageSrc(raw: string | undefined): string {
  ...
  // اگر از روت شروع شده باشد (مثل /uploads/xxx.jpg)
  if (src.startsWith('/')) {
    return src
  }

  // اگر اشتباهی public/ اولش باشد
  if (src.startsWith('public/')) {
    src = src.slice('public/'.length)
  }

  // اگر با uploads/ شروع شود
  if (src.startsWith('uploads/')) {
    src = src.slice('uploads/'.length)
  }

  // در نهایت همه چیز را به /uploads/filename تبدیل می‌کنیم
  return `/uploads/${src}`
}
